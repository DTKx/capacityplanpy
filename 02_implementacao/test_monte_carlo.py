...
def main():
    ...
    stock = np.array([[6, 0.0, 0.0, 22], [6, 0.0, 4, 26], [6, 0.0, 4, 26]])  # Stock of available products, Each column represents a product, each row a month.
    ...
    for j in range(0, num_sim_monte_carlo):  # loop per number of monte carlo simulations
        demand_j = np.zeros(shape=demand_triangular.shape)
        for k in range(0, triangular_len):  # loop per values to simulate
            print(demand_j[index_demand_not_0][k])  # print 0.0
            print(dict_demand_values_simulations[k][j])  # print 4.81
            demand_j[index_demand_not_0][k] = dict_demand_values_simulations[k][j]  # Populates all simulated values from simulations in each 1000 demand array
            # 2) Issue 2 demand_j[index_demand_not_0][k] is being set to 4.81
            print(demand_j[index_demand_not_0][k])  # print 0.0; EXPECTED 4.81
        sums_available_simulations[j] = np.sum(demand_j - stock)  # Stores available for each simulation
